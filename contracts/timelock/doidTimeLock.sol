...
contract DoidTimeLock is OwnableUpgradeable {
    struct StakingInfo {
        bool queued;
        uint amount;
        uint timestamp;
    ...
    event Queue(address indexed target, uint value, uint timestamp);
    event Execute(address indexed target, uint value, uint timestamp);
    event EmergencyWithdraw(address user, uint amount);

    uint public constant MIN_DELAY = 99 * 24 * 3600; // seconds
    uint public constant MAX_DELAY = 365 * 24 * 3600; // seconds
    // uint public constant GRACE_PERIOD = 365 * 24 * 3600; // seconds
    ...
    function getQueue(
        address _user
    ) public view returns (bool _queued, uint _amount, uint _timestamp) {
    ...
    function queue(uint _timestamp) external payable {
        ...
        userStaking[msg.sender] = StakingInfo({
            queued: true,
            amount: msg.value,
